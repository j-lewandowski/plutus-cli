...
func (r *Repository) migrate() error {
	queries := []string{
		...
		`CREATE TABLE IF NOT EXISTS "sell" (
			id INTEGER PRIMARY KEY AUTOINCREMENT,
			sell_date DATE DEFAULT(datetime(current_timestamp)),
			sell_amount_in_eurocents INTEGER NOT NULL,
			sell_volume INTEGER NOT NULL DEFAULT 0,
			sell_volume_precision INTEGER NOT NULL DEFAULT 0
		);`,
	}
	...
	r.conn.Exec(`UPDATE sell SET sell_date = substr(sell_date, 1, 10) WHERE length(sell_date) > 10;`)
